...
process_nodes(xmlTextReaderPtr reader, envelope *bmd_envelope, payload *bmd_payload)
{
    ...
    if (xmlStrcmp(MessageID, name) == 0)
    {
        ...
        bmd_envelop->MessageID = xmlStrdup(value);
    ...
    {
        value = get_node_data(reader);
        bmd_envelop->MessageType = xmlStrdup(value);
    ...
    {
        value = get_node_data(reader);
        bmd_envelop->Sender = xmlStrdup(value);
        //printf("Sender %s \n", bmd_envelope->Sender);
    }
    ...
    {
        value = get_node_data(reader);
        bmd_envelop->Destination = xmlStrdup(value);
    ...
    {
        value = get_node_data(reader);
        bmd_envelop->Signature = xmlStrdup(value);
    }
    ...
    {
        value = get_node_data(reader);
        bmd_envelop->ReferenceID = xmlStrdup(value);
    }

    if (xmlStrcmp(CreationDateTime, name) == 0)
    {
        value = get_node_data(reader);
        bmd_envelop->CreationDateTime = xmlStrdup(value);
    }
...
static void
streamFile(const char *bmd_file_path, envelop *bmd_envelop, payload *bmd_payload)
{
...
static envelop *get_envelop_struct()
{
    envelope *bmd_envelop = (envelope *)malloc(sizeof(envelop));
    return bmd_envelop;
}
...
BMD *process_xml(char *bmd_file_path)
{
    ...
    bmd->bmd_envelop = get_envelop_struct();
    bmd->bmd_payload = get_payload_struct();

    streamFile(bmd_file_path, bmd->bmd_envelop, bmd->bmd_payload);
