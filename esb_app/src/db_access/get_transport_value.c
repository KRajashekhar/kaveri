...
MYSQL *conn =mysql_init(NULL);
    ...
    return_value = snprintf(quer,STRING_SIZE,"SELECT config_value FROM \
     transport_config  WHERE route_id = %d ",route_id);
     
    char query[return_value+1];
    return_value = snprintf(query,return_value+1, "SELECT config_value FROM \
